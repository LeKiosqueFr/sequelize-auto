      ...
      _.each(fields, function(field, i){
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            text[table] += spaces + spaces + spaces + "adl.hasOne(product, { as: '"+_tables[table][field][attr].target_table+"', foreignKey: '"+_tables[table][field][attr].target_column+"'});\n";
           }
          }
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
           ...
           text[table] += spaces + spaces + spaces + "var p = sequelize.import('./"+_tables[table][field][attr].target_table+".model.js');\n";
            ...
            text[table] += spaces + spaces + spaces + spaces + spaces+ "trace(rows);\n";                  
            text[table] += spaces + spaces + spaces + spaces + "});\n";                  
            text[table] += spaces + spaces + spaces + "}\n";
           }
          }
            ...
            text[table] += spaces + spaces + "m.findAllWithAll = function(limit) {\n";
            text[table] += spaces + spaces + "var pp=[];\n";
                ...
                _.each(fieldAttr, function(attr, x){
                  ...
                  if (attr === "foreignKey") {
                   ...
                   text[table] += spaces + spaces + spaces + "var p = sequelize.import('./"+_tables[table][field][attr].target_table+".model.js');\n";
                    text[table] += spaces + spaces + spaces + "pp.push({ model: p, as: 'p' });\n";
                   }
                  }
